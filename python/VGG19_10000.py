...
historyfilename ='VGG19_10000_Modelhistory_'+filetime+'.json'
...
optfile = 'VGG19_10000_Modeloutput_'+filetime+'.txt'
file = open(optfile, "a+")
...
cmfile='VGG19_10000_confusionMatrix_'+filetime+'.json' 
df=pd.DataFrame(cnf_matrix)
df.to_json(cmfile, orient='records', lines=True)

predjson='VGG19_10000_Prediction_'+filetime+'.json' 
df=pd.DataFrame(y_pred)
df.to_json(predjson, orient='records', lines=True)

rptjson = 'VGG19_10000_ClassificationReport_'+filetime+'.json' 
df=pd.DataFrame(cnf_matrix)
df.to_json(rptjson, orient='records', lines=True)

sysoptfile ='VGG19_10000_ClassificationSystemEnvironment_'+filetime+'.txt'
import subprocess
...
plotopt= 'VGG19_10000_ClassificationModelImage_'+filetime+'.png'
...
gpuplt=gpu.plot()
gpuplt=pyplot.show()
gpuplt= 'VGG19_10000_ClassificationGPUImage_'+filetime+'.png'
