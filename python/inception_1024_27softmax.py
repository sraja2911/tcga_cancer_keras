...
#################################################################################################################################
# Added by Raj, 30th July, 2018
# Solution to reover from the error : layout: BatchDepthYX} to cudnn tensor descriptor: CUDNN_STATUS_BAD_PARAM
# https://github.com/keras-team/keras/issues/8123
# ------------------- pass the the template model to gpu ---------------------
#if ngpus > 1:
#    gpu_model = multi_gpu_model(model,ngpus)
#gpu_model.compile(loss='categorical_crossentropy', optimizer='adadelta', metrics=['accuracy'])
#################################################################################################################################
...
epochfilename ='Inception_1024_ClassificationEpochRuntime_'+filetime+'.json'
df=pd.DataFrame(times)
df.to_json(epochfilename, orient='records', lines=True)


modelfilename=str(now.year)+str(now.month)+str(now.day) 
modelfilename='Inception_1024_'+modelfilename+'.h5'
...
historyfilename ='Inception_1024_Modelhistory_'+filetime+'.json'
...
optfile = 'Inception_1024_Modeloutput_'+filetime+'.txt'
file = open(optfile, "a+")
...
cmfile='Inception_1024_ConfusionMatrix_'+filetime+'.json' 
df=pd.DataFrame(cnf_matrix)
df.to_json(cmfile, orient='records', lines=True)

predjson='Inception_1024_Prediction_'+filetime+'.json' 
df=pd.DataFrame(y_pred)
df.to_json(predjson, orient='records', lines=True)

rptjson = 'Inception_1024_ClassificationReport_'+filetime+'.json' 
df=pd.DataFrame(cnf_matrix)
df.to_json(rptjson, orient='records', lines=True)

sysoptfile ='Inception_1024_ClassificationSystemEnvironment_'+filetime+'.txt'
import subprocess
...
plotopt= 'Inception_1024_ClassificationModelImage_'+filetime+'.png'
...
gpuplt=gpu.plot()
gpuplt=pyplot.show()
gpuplt= 'Inception_1024_ClassificationGPUImage_'+filetime+'.png'
